...
private:
  ...
  void threadDispatcher(const size_t id)
  {
    ...
    for(; running && ros::ok();)
    {
      ...
      //ROS_ERROR("MAIN LOOOOOOOOOOOOOOOOOOP");
      for(size_t i = 0; i < 2; ++i)
      {
      //ROS_ERROR("HECK first: %d", checkFirst);
        if(i == checkFirst)
        {
         // ROS_ERROR("IIIIIIIIIIIIIIIIIIIIIIIIIN There");
          if(nextIrDepth && true)//lockIrDepth.try_lock())
          {
            nextIrDepth = false;
            receiveIrDepth();
            processedFrame = true;
          ...
          //ROS_ERROR("next ir Depth trueeeeeeeeeeeeeeeeeeeeeeeeeee");
          }
        }
        else
        {
  ...
  bool receiveIrDepth()
  {
    ...
    //ROS_ERROR("RECIVE IR DEPTH wati to reveife");
...
//    lockIrDepth.unlock();
    ...
    //ROS_ERROR("RECIVE IR DEPTH RECEIVEDDD");
    updateStatus(status);
    ...
    //ROS_ERROR("RECIVE IR DEPTH PROCESSED");
...
///      std::cerr<<"NO COPY" << std::endl;
 //   }
    ...
    //ROS_ERROR("RECIVE IR DEPTH OUBBED");
